...
export class OHCMCEngine
  implements TournamentEngine<OHCMCState, OHCMCAction, OHCMCPhase>
{
  readonly id = "ochcmc";
  readonly name = "OHCMC";
  ...
  startMatch(state: OHCMCState): Result<OHCMCState> {
    return success({ ...state, currentPhase: null }, [
      { type: "match_started", match_code: state.matchCode },
    ]);
  }

  canStartPhase(state: OHCMCState, phase: OHCMCPhase): boolean {
    return (
      this.phases.includes(phase) && !state.completedPhases.includes(phase)
    );
  }

  startPhase(state: OHCMCState, phase: OHCMCPhase): Result<OHCMCState> {
    if (!this.canStartPhase(state, phase))
      return failure("INVALID_PHASE", `Cannot start OHCMC phase: ${phase}`);
    return success({ ...state, currentPhase: phase }, [
  ...
  endPhase(state: OHCMCState, phase: OHCMCPhase): Result<OHCMCState> {
    if (state.currentPhase !== phase)
      return failure("PHASE_NOT_ACTIVE", `OHCMC phase is not active: ${phase}`);
    return success(
      {
        ...state,
        currentPhase: null,
  ...
  handleAction(state: OHCMCState, action: OHCMCAction): Result<OHCMCState> {
    if (state.currentPhase !== action.phase)
      return failure("PHASE_NOT_ACTIVE", "Action phase is not active");
