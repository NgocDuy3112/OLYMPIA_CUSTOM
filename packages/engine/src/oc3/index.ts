...
export class OC3Engine implements TournamentEngine<OC3State, OC3Action, OC3Phase> {
  readonly id: string = OC3_CONFIG.id
  readonly name: string = OC3_CONFIG.name
  readonly phases: readonly OC3Phase[] = ['kdc', 'kdr', 'bp', 'vdc', 'vdr', 'gm']
  ...
  private handleActionLegacy(state: OC3State, action: OC3Action): {
    state: OC3State
  ...
  private handleBuzz(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
  ...
  private handleAnswer(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
  ...
  private handleSendQuestion(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
  ...
  private handleStartTimer(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
    const { payload } = action
    const timer = {
  ...
  private handleClearQuestion(state: OC3State, _action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
    return {
      state: {
  ...
  private handleVeDichPower(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
    const { userCode, payload } = action
  ...
  private handleKeywordSubmit(state: OC3State, action: OC3Action): {
    state: OC3State; broadcasts: BroadcastPayload[]; scoreDeltas: ScoreDelta[]
  } {
    const { userCode, payload } = action
