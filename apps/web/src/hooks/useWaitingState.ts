  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      switch (message.type) {
        case "send_room_info":
          setMatchName(
            typeof message.match_name === "string" ? message.match_name : "",
          );
        ...
        case "score_chart_snapshot":
          if (Array.isArray(message.question_labels))
            setQuestionLabels(message.question_labels.map(String));
          if (message.chart_data && typeof message.chart_data === "object") {
            setChartData(
              message.chart_data as Record<
                string,
                {
