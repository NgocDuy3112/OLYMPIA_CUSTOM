...
export async function resolveS3Media(key: string): Promise<string | null> {
  if (!isS3Key(key)) return key;
  const cached = presignCache.get(key);
  if (cached) return cached;
  const response = await fetch(`${API_BASE_URL}/media/presign-batch/`, {
    method: "POST",
    headers: {
  ...
  if (url) presignCache.set(key, url);
  return url;
}
...
export function useS3Media(
  ...
  useEffect(() => {
    ...
    if (cached) {
      setState({
        src: cached,
        mimeType: mimeFromPath(mediaUrl),
        loading: false,
        error: null,
      });
      return;
