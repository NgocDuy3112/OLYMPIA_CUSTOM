...
export function useAudiencePlayers() {
  ...
  const applyScoreUpdate = useCallback((message: WebSocketMessage) => {
    if (
      (typeof message.user_code === "string" ||
        typeof message.user_code === "number") &&
    ...
    setPlayers((previous) =>
      previous.map((player) => {
        const answer = answers.find(
          (item) => String(item.user_code) === player.playerCode,
        );
        ...
        return {
          ...
          playerTimestamp:
            message.type === "send_keyword_answers"
              ? undefined
              : (answer.timestamp ?? player.playerTimestamp),
          playerKeywordCluesOpened:
            answer.clues_opened ?? player.playerKeywordCluesOpened,
        };
      }),
    );
  }, []);
  ...
  const applyKeywordSubmit = useCallback((message: WebSocketMessage) => {
    ...
    const userCode = String(message.user_code);
    const cluesOpened =
      typeof message.clues_opened === "number"
        ? message.clues_opened
        : undefined;
    setPlayers((previous) =>
      previous.map((player) =>
        player.playerCode === userCode
          ? {
              ...
              playerKeywordCluesOpened:
                cluesOpened ?? player.playerKeywordCluesOpened,
            }
          : player,
      ),
    );
  ...
  const applyPlayerPower = useCallback(
    (userCode: string, power: "star" | "shield") => {
      setPlayers((previous) =>
        previous.map((player) =>
          player.playerCode === userCode
            ? { ...player, playerPower: power }
            : player,
        ),
      );
    },
  ...
  const applyAfkUpdate = useCallback((message: WebSocketMessage) => {
    if (
      typeof message.user_code !== "string" &&
      typeof message.user_code !== "number"
    )
      return;
    const userCode = String(message.user_code);
    const isAfk = message.status === "afk" || message.afk === true;
    setPlayers((previous) =>
      previous.map((player) =>
        player.playerCode === userCode
          ? { ...player, playerAfk: isAfk }
          : player,
      ),
    );
  }, []);
  ...
  const applyWrongAttempt = useCallback((message: WebSocketMessage) => {
    ...
    setPlayers((previous) =>
      previous.map((player) =>
        player.playerCode === userCode
          ? { ...player, playerWrongAttempts: attempts }
          : player,
      ),
    );
  }, []);
