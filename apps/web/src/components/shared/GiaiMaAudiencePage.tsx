...
function isMediaFilename(value: string): boolean {
  return /\.(mp3|ogg|wav|aac|m4a|mp4|webm|mov|jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(
    value.trim(),
  );
}
...
const PlayerClueCard: React.FC<PlayerClueCardProps> = ({
  index,
  state,
  hintContent,
}) => {
    ...
    queueMicrotask(() => {
      ...
      switch (msg?.type) {
        ...
        case "keyword_submit": {
          const { user_code } = msg;
          if (user_code) {
            setKeywordSubmittedCodes(
              (prev) => new Set([...prev, user_code as string]),
            );
        ...
        case "show_hint": {
          ...
          const questionCodeMatch = String(msg.question_code ?? "").match(
            /(\d+)\s*$/,
          );
          const questionCodeIdx = questionCodeMatch
            ? Number(questionCodeMatch[1]) - 1
            : null;
          const hasQuestionCodeIdx =
            Number.isInteger(questionCodeIdx) &&
            questionCodeIdx !== null &&
            questionCodeIdx >= 0 &&
            questionCodeIdx < CLUE_COUNT;
          const hasExplicitIdx =
            Number.isInteger(explicitIdx) &&
            explicitIdx >= 0 &&
            explicitIdx < CLUE_COUNT;
          const idx = hasExplicitIdx
            ? explicitIdx
            : hasQuestionCodeIdx
              ? questionCodeIdx
