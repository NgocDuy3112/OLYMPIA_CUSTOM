    ...
    queueMicrotask(() => {
      ...
      switch (msg?.type) {
        case "send_players_info":
          ...
          {
            const current = (msg.players as RawPlayer[] | undefined)?.find(
              (player) => player.is_current,
            );
            setCurrentPlayerCode(
              current ? String(current.user_code ?? "") : "",
            );
          }
          break;
        ...
        case "buzzer_winner": {
          ...
          lastBuzzerQuestionRef.current = winner
            ? (msg.question_code ?? null)
            : null;
          setPlayers((prev) =>
            prev.map((p) => ({
              ...p,
    ...
    >
      <>
        <AQuestionBoard
          ...
          timerDuration={
            answeringWindowTimer > 0 ? answeringWindowTimer : timer
          }
