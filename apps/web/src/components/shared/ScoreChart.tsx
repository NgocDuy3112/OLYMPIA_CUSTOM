...
function labelFor(code: string) {
  ...
  if (parts[0] === "KD" && parts.length >= 2)
    return parts[1] === "C"
      ? `KĐ_C_${parts.slice(2).join("_")}`
      : `KĐ_${parts.slice(1).join("_")}`;
  return value
    .replace("KD_C", "KĐC")
    .replace("GM", "GM")
    .replace("BP", "BP")
  ...
  if (parts[0] === "VD") {
    ...
    const questionPoints =
      points === 20 || points === 30 || points === 40 || points === 50
        ? points
        : codePoints;
    const question = [20, 30, 40, 50].includes(questionPoints)
      ? `${questionPoints} điểm`
      : "1";
...
export default function ScoreChart({
  ...
  const activePlayerCode =
    hoveredPlayerCode ?? hoveredPoint?.playerCode ?? null;
  const updateHoveredPoint = (next: HoveredPoint) => {
    setHoveredPoint((previous) =>
      previous?.playerCode === next?.playerCode &&
  ...
  const colorForPlayer = (code: string, fallbackIndex = 0) => {
    const playerIndex = players.findIndex(
      (player) => String(player.playerCode) === String(code),
    );
    return COLORS[
      (playerIndex >= 0 ? playerIndex : fallbackIndex) % COLORS.length
    ];
  };
  ...
  const recordedGroups = new Set(
    recordedLabels.map((code) => groupFor(labelFor(code))),
  );
  const labels = Array.from(
    new Set([
      ...questionLabels
        .filter((code) => recordedGroups.has(groupFor(labelFor(code))))
        .map(labelFor),
  ...
  labels.sort((left, right) => {
    const groupDifference =
      groupOrder.indexOf(groupFor(left)) - groupOrder.indexOf(groupFor(right));
    ...
    const questionNumber = (label: string) => {
      ...
      const match =
        label.match(/(?:Câu|Gợi ý)\s+(\d+)/) ?? label.match(/_(\d+)$/);
  ...
  const scoreValues = codes.flatMap((code) =>
    chartData[code].map((point) => point.cumulative_score),
  );
  const minScore = Math.min(0, ...scoreValues);
  const maxScore = Math.max(0, ...scoreValues);
  const scoreRange = Math.max(1, maxScore - minScore);
  const groups = labels.reduce<{ name: string; start: number; end: number }[]>(
    (result, code, index) => {
  ...
  const x = (index: number) =>
    chartLeft +
    (index * (chartRight - chartLeft)) / Math.max(1, labels.length - 1);
    ...
    <section className="w-full max-w-3xl rounded-xl border-2 border-blue-600 bg-blue-950/70 p-3 shadow-xl">
      ...
      <div className="overflow-x-auto">
        <svg
          ...
          role="img"
          aria-label="Biểu đồ diễn biến điểm"
          onMouseLeave={() => updateHoveredPoint(null)}
        ...
        >
          ...
          {groups.map((group, index) => (
            <g key={group.name}>
              <rect
                x={group.start === 0 ? padding.left : x(group.start) - 12}
                y={padding.top}
                width={
                  (group.end === labels.length - 1
                    ? width - padding.right
                    : x(group.end + 1) - 12) -
                  (group.start === 0 ? padding.left : x(group.start) - 12)
                }
          ...
          {codes.map((code, playerIndex) => {
            ...
            const pointDetails = labels.map((label) =>
              points
                .filter((point) => labelFor(point.question_code) === label)
                .reduce<
                  ChartPoint | undefined
                >((latest, point) => (!latest || point.cumulative_score >= latest.cumulative_score ? point : latest), undefined),
            );
            let lastScore = 0;
            ...
            const path = values.reduce(
              (result, value, index) =>
                value == null
                  ? result
                  : `${result}${result ? " L" : "M"}${x(index)} ${y(value)}`,
              "",
            );
            const isDimmed =
              activePlayerCode !== null && activePlayerCode !== code;
            return (
              <g key={code} opacity={isDimmed ? (focusMode ? 0 : 0.2) : 1}>
                <path
                  d={path}
                  fill="none"
                  stroke={colorForPlayer(code, playerIndex)}
                  strokeWidth={activePlayerCode === code ? "5" : "3"}
                  ...
                  onMouseEnter={() =>
                    updateHoveredPoint({
                      playerCode: code,
                      index:
                        hoveredPoint?.index ?? Math.max(0, values.length - 1),
                    })
                  }
                />
                  ...
                  value == null ? null : (
                    <g key={`${code}-${index}`}>
                      <circle
                        ...
                        fill="transparent"
                        onMouseEnter={() =>
                          updateHoveredPoint({ playerCode: code, index })
                        }
                      />
                      <circle
                        cx={x(index)}
          ...
          {labels.map((_, index) => {
            const firstCode = codes.find((code) =>
              chartData[code].some(
                (point) => labelFor(point.question_code) === labels[index],
              ),
            );
            if (!firstCode) return null;
            const step =
              (chartRight - chartLeft) / Math.max(1, labels.length - 1);
            return (
              <rect
                key={`hit-${index}`}
                ...
                pointerEvents="all"
                onMouseEnter={() =>
                  updateHoveredPoint({ playerCode: firstCode, index })
                }
              />
            );
          })}
          {hoveredPoint
            ? (() => {
                const rows = codes.map((code, playerIndex) => {
                  const points = chartData[code];
                  const detail = labels
                    .slice(0, hoveredPoint.index + 1)
                    .map((label) =>
                      points.find(
                        (point) => labelFor(point.question_code) === label,
                      ),
                    )
                    .reverse()
                    .find(Boolean);
                  ...
                  return {
                    ...
                    detail,
                    current,
                    value: detail?.cumulative_score ?? 0,
                  };
                });
                const active = rows.find(
                  (row) => row.code === hoveredPoint.playerCode,
                );
                if (!active?.detail) return null;
                const hoveredLabel = labels[hoveredPoint.index];
                const titleCode =
                  questionLabels.find(
                    (code) => labelFor(code) === hoveredLabel,
                  ) ??
                  codes
                    .flatMap((code) => chartData[code])
                    .find(
                      (point) => labelFor(point.question_code) === hoveredLabel,
                    )?.question_code ??
                  hoveredLabel;
                const titlePoint = recordedPoints.find(
                  (point) => labelFor(point.question_code) === hoveredLabel,
                );
                ...
                return (
                  <foreignObject
                    ...
                    x={Math.min(
                      width - 230,
                      Math.max(8, x(hoveredPoint.index) - 108),
                    )}
                    y={Math.max(
                      8,
                      y(Math.max(...rows.map((row) => row.value))) - 112,
                    )}
                    width="222"
                    height="104"
                    pointerEvents="none"
                  >
                    <div className="rounded-lg border border-blue-300/60 bg-[#061226]/95 px-3 py-2 text-left text-xs text-blue-50 shadow-xl">
                      ...
                      {rows.map((row) => (
                        <div
                          key={row.code}
                          className={`grid grid-cols-[1fr_3rem_4rem] items-center gap-2 leading-4 ${row.code === activePlayerCode ? "" : "opacity-40"}`}
                        >
                          <span
                            className="truncate"
                            style={{
                              color: colorForPlayer(row.code, row.playerIndex),
                            }}
                          >
                            {players.find(
                              (player) => player.playerCode === row.code,
                            )?.playerName ?? row.code}
                          </span>
                          <b className="text-right">
                            {row.current?.points ?? 0}
                          </b>
                          <b
