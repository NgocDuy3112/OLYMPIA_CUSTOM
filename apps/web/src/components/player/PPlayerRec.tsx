...
const PPlayerRec: React.FC<PPlayerRecProps> = ({
  ...
  if (isBuzzerWinner) {
    content = (
      <>
        <p
          className={`px-2 rounded-md font-bold text-wrap ${isAnswered ? answerClasses : "text-white"}`}
        >
          <Zap size={40} />
        </p>
      </>
    ...
    content = (
      <>
        <p
          className={`px-2 rounded-md text-[18px] font-bold text-wrap ${isAnswered ? answerClasses : "text-white"}`}
        >
          {displayAnswer}
        </p>
    ...
    >
      <div className="flex justify-between items-center w-full">
        <p className="text-[28px] font-bold font-[SVN-Gratelos_Display] uppercase truncate text-left max-w-[80%] flex items-center gap-2">
          ...
          {isCurrent && <Mic size={20} className="text-white inline-block" />}
