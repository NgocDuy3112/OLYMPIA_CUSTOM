...
const AdminKhoiDongChungView = () => {
  ...
  const {
    ...
    hasQuestionSelected, toggleSelectedPlayer, loadQuestion, sendQuestionToPlayers,
    startTimer, showAnswers, calculateAndBroadcastScore, handleEditScore, endRound,
    setPlayers, sendPlayersSnapshot,
  } = useGameRound({ round: "kdc", questionPrefix: QUESTION_PREFIX, timeLimit: TIME_LIMIT, timerPhase: "kdc" });
  ...
  useEffect(() => {
    ...
    if (targetIndex !== lastAutoAdvancedIndexRef.current) {
      ...
      void loadQuestion(targetIndex).then(q => { if (q) void sendQuestionToPlayers(targetIndex, q); });
    }
  }, [isTimerRunning, timer, setCurrentQuestionIndex, setPlayers, loadQuestion, sendQuestionToPlayers]);

  useEffect(() => {
    if (timer <= 0) { lastAutoAdvancedIndexRef.current = 0; hasStartedRoundTimerRef.current = false; }
  }, [timer]);
  ...
  const handleStartRound = useCallback(async () => {
    ...
    await startTimer(1);
    loadQuestion(1).then(q => { if (q) void sendQuestionToPlayers(1, q); });
  }, [isTimerRunning, startTimer, loadQuestion, sendQuestionToPlayers, sendPlayersSnapshot]);
      ...
      {Array.from({ length: MAX_QUESTION_INDEX }).map((_, idx) => {
        ...
        return (
          <button key={idx} type="button" disabled={isTimerRunning}
            onClick={() => { if (!isTimerRunning) { setCurrentQuestionIndex(idx + 1); loadQuestion(idx + 1).then(q => { if (q) void sendQuestionToPlayers(idx + 1, q); }); } }}
  ...
  return (
    <ANewBaseLayout
      title="KHỞI ĐỘNG - LƯỢT CHUNG" players={players} selectedPlayerCodes={selectedPlayerCodes}
      ...
      playerActions={
        <>
          <AControlButton onClick={handleStartRound} disabled={isTimerRunning || hasStartedRoundTimerRef.current}>
            <AlarmClockCheck size={18} /><span className="ml-2 font-bold">ĐẾM GIỜ</span>
          </AControlButton>
          <AControlButton onClick={() => calculateAndBroadcastScore("kdc_correct")} disabled={selectedPlayerCodes.length === 0 || hasAddedScore || isTimerRunning}>
    ...
    >
      <AQuestionBoard title="KHỞI ĐỘNG - LƯỢT CHUNG" question={currentQuestion} timerDuration={timer}
...
const PlayerKhoiDongChungView = () => {
  ...
  useEffect(() => { setAnswer(""); }, [currentQuestionIndex]);

  const handleSubmitAnswer = useCallback(async () => {
    const trimmed = answer.trim();
    if (!trimmed || !isConnected || timer <= 0 || !currentQuestion.questionCode) return;
    ...
    setAnswer("");
  }, [answer, currentQuestion.questionCode, getElapsedSeconds, isConnected, matchCode, playerCode, sendMessage, setPlayers, timeLimit, timer]);
  ...
  return (
    <PBasePageLayout players={displayPlayers} currentPlayerCode={playerCode}>
      <PQuestionBoard title="KHỞI ĐỘNG - LƯỢT CHUNG" question={currentQuestion} timerDuration={timer}
        controls={{ variant: 'numbers', count: 6, activeIndices: currentQuestionIndex > 0 ? [currentQuestionIndex - 1] : [] }}
      />
      <PAnswerBox answer={answer} setAnswer={setAnswer} isDisabled={!isConnected || timer <= 0}
