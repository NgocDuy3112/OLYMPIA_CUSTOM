...
const AGameManagingPage = () => {
  ...
  const [scoreboard, setScoreboard] = useState<
    { user_code: string; user_name: string; cumulative_score: number }[]
  >([]);
  ...
  const fetchScoreboard = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/scoreboard/${encodeURIComponent(code)}`,
        {
          headers: authHeaders(),
        },
      );
        ...
        <div className="col-span-2 bg-blue-900/60 ring-4 ring-blue-600 rounded-xl p-5 flex flex-col gap-4 overflow-hidden">
          <div className="flex items-center justify-between">
            ...
            >
              <RefreshCw
                size={14}
                className={scoreboardLoading ? "animate-spin" : ""}
              />{" "}
              Tải bảng điểm
            </button>
          </div>
          ...
          {!matchCode ? (
            <p className="text-gray-400 text-sm">
              Nhập mã trận đấu ở phần "Tạo trận đấu" để xem và sửa điểm.
            </p>
          ) : scoreboard.length === 0 && !scoreboardLoading ? (
            <p className="text-gray-400 text-sm">
              Chưa có điểm. Bấm "Tải bảng điểm" để lấy dữ liệu.
            </p>
          ) : (
            <div className="overflow-y-auto flex-1 -mr-2 pr-2">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {scoreboard.map((entry, idx) => (
                    <tr
                      ...
                      className="border-b border-blue-800/50 hover:bg-blue-800/40 transition-colors"
                    >
                      <td className="py-2 px-2 font-bold text-blue-200">
                        #{idx + 1}
                      </td>
                      <td className="py-2 px-2 font-mono text-xs">
                        {entry.user_code}
                      ...
                      <td className="py-2 px-2 text-right">
                        {editingScoreUser === entry.user_code ? (
                          ...
                          <button
                            onClick={() => {
                              setEditingScoreUser(entry.user_code);
                              setEditScoreValue(String(entry.cumulative_score));
                            }}
                            className="p-1.5 rounded bg-white-600/70 hover:bg-white-500 transition-colors"
                            title="Sửa điểm"
                          >
