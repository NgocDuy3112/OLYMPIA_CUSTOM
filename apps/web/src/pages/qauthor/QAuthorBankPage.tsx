...
const QAuthorBankPage = () => {
  ...
  // Upload media cho 1 câu bank: presign-put questions/<bankCode>/<file> rồi
  // PATCH mediaUrl vào bank. Excel chỉ nhập text, media chèn sau bước này.
  const uploadMedia = useCallback(async (q: BankData, file: File) => {
    ...
    try {
      const key = `questions/${q.bank_code}/${Date.now()}_${file.name}`;
  ...
  return (
    <div className="flex flex-col gap-4 p-3 sm:p-4 lg:p-6 min-h-screen text-white">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-5 flex flex-col gap-4">
        <h2 className="flex items-center gap-2 text-xl font-bold text-green-300">
          <Database size={20} /> Bank câu hỏi (QB_*)
        </h2>
...
export default QAuthorBankPage;
