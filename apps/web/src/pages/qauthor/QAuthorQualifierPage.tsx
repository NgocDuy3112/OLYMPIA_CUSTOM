  ...
  const fetchQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (json.status === "success" && Array.isArray(json.data)) {
        setQuestions(
          (json.data as Record<string, unknown>[]).map(toQuestion),
        );
      } else {
        setQuestions([]);
        logger.warn("Fetch qualifier failed:", json.message);
      }
    } catch (err) {
  ...
  const fetchStandings = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}${base()}/standings?limit=16`,
        { credentials: "include" },
      );
      const json: ApiResponse = await res.json();
      if (json.status === "success" && Array.isArray(json.data)) {
    ...
    <div className="flex flex-col gap-4 p-3 sm:p-4 lg:p-6 min-h-screen text-white">
      ...
      <div className="bg-blue-900/60 ring-4 ring-blue-600 rounded-xl p-5 flex flex-col gap-4">
        <h2 className="flex items-center gap-2 text-xl font-bold text-blue-300">
          <ListOrdered size={20} /> Vòng loại — 16 câu / giải
        </h2>
      ...
      <div className="bg-blue-900/60 ring-4 ring-blue-600 rounded-xl p-5 flex flex-col gap-4">
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {questions.map((q) => (
                <tr key={q.id} className="border-b border-blue-800/50 align-top">
      ...
      <div className="bg-blue-900/60 ring-4 ring-blue-600 rounded-xl p-5 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-blue-700 hover:bg-blue-600 disabled:opacity-50 text-xs"
          >
            Làm mới
          </button>
        </div>
        {standings.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {standings.map((s) => (
                <tr key={s.playerId} className="border-b border-blue-800/50">
