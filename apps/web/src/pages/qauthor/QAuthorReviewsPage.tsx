...
const QAuthorReviewsPage = () => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {reviews.length === 0 && (
        <p className="text-sm text-gray-500">Không có review chờ duyệt.</p>
      )}
      {reviews.map((r) => {
          ...
          <div key={r.id} className="rounded-xl bg-white/5 border border-white/10 p-4 flex flex-col gap-3">
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 text-xs text-white"
              >
              ...
              >
                {saving === r.id ? "Đang chốt..." : "Xác nhận"}
              </button>
            </div>
          </div>
        );
