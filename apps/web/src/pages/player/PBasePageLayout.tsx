...
import { PlayerVoiceReceiver } from "@/components/player/PlayerVoiceReceiver";
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      {role === "player" && (
        <>
          ...
          <PlayerVoiceReceiver publisherCode="mc" />
        </>
      )}
      {role === "mc" && <VoicePublisher userCode="mc" />}
      <HeaderBar
        matchCode={matchCode}
        phase={currentPhase}
        ...
        {players.map((p) =>
          role === "mc" ? (
            <PlayerPanel
              key={p.playerCode}
              player={p}
              isActive={p.playerIsTurn ?? p.playerCode === effectiveCurrentCode}
              isCurrent={
                p.playerIsTurn ?? p.playerCode === effectiveCurrentCode
              }
            />
          ) : (
            <PPlayerRec
