...
export const isMatchCode = (value: unknown): value is string =>
  typeof value === "string" && MATCH_PATTERN.test(value);
export const isQuestionCode = (value: unknown): value is string =>
  typeof value === "string" && QUESTION_PATTERN.test(value);
export const isUserCode = (value: unknown): value is string =>
  typeof value === "string" && USER_PATTERN.test(value);
export const isRole = (value: unknown): value is Role =>
  typeof value === "string" && (ROLES as readonly string[]).includes(value);
export const isQualifierOption = (value: unknown): value is QualifierOption =>
  typeof value === "string" &&
  (QUALIFIER_OPTIONS as readonly string[]).includes(value.toUpperCase());
export const isRoundNumber = (value: unknown): value is number =>
  typeof value === "number" &&
  Number.isInteger(value) &&
  value >= 1 &&
  value <= 5;
export const isMultipleOfFive = (value: unknown): value is number =>
  typeof value === "number" && Number.isInteger(value) && value % 5 === 0;
...
export function validateScoreEvent(input: {
  match_code: unknown;
  question_code: unknown;
  user_codes: unknown[];
}): void {
  ...
  assertQuestionCode(input.question_code);
  if (
    !Array.isArray(input.user_codes) ||
    input.user_codes.some((code) => !isUserCode(code))
  ) {
    throw new Error("user_codes must contain only player codes");
  }
}

export function validateScoreAdjust(input: {
  match_code: unknown;
  user_code: unknown;
  ...
  assertMatchCode(input.match_code);
  assertUserCode(input.user_code);
  if (input.question_code !== undefined && input.question_code !== null)
    assertQuestionCode(input.question_code);
  if (input.new_score === null || input.new_score === undefined) {
    if (
      input.question_code === undefined ||
      input.question_code === null ||
      input.points === null ||
  ...
  if (
    input.new_score !== undefined &&
    input.new_score !== null &&
    !isMultipleOfFive(input.new_score)
  )
    throw new Error("new_score must be a multiple of 5");
  if (
    input.points !== undefined &&
    input.points !== null &&
    !isMultipleOfFive(input.points)
  )
    throw new Error("points must be a multiple of 5");
}
...
export function validateRecordInput(input: {
  ...
  assertUserCode(input.user_code);
  assertQuestionCode(input.question_code);
  if (!isMultipleOfFive(input.points))
    throw new Error("points must be a multiple of 5");
}

export function validateQualifierScore(input: {
  match_code: unknown;
  question_code: unknown;
  ...
  assertMatchCode(input.match_code);
  assertQuestionCode(input.question_code);
  if (!isQualifierOption(input.correct_answer))
    throw new Error("correct_answer must be one of A, B, C, D, E, F");
  if (!isRoundNumber(input.round_number))
    throw new Error("round_number must be between 1 and 5");
}

export function validateEndRound(input: {
  match_code: unknown;
  round_number: unknown;
  ...
  if (!isRoundNumber(input.round_number))
    throw new Error("round_number must be between 1 and 5");
  if (
    input.advance_count !== undefined &&
    input.advance_count !== null &&
    (typeof input.advance_count !== "number" ||
      !Number.isInteger(input.advance_count) ||
      input.advance_count < 0)
  ) {
...
export function validateMediaUrl(value: unknown): void {
  ...
  if (
    !/^https?:\/\//.test(first) &&
    !(isMatchCode(first.split("/")[0]) && first.includes("/"))
  ) {
