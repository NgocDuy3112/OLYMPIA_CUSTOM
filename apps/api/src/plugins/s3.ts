...
declare module 'fastify' {
  interface FastifyInstance {
    s3: S3Client | null
    s3Upload: (key: string, body: Buffer, contentType?: string) => Promise<void>
...
async function s3Plugin(app: FastifyInstance) {
  ...
  if (!env.S3_BUCKET_NAME || !env.S3_ACCESS_KEY_ID) {
    ...
    app.decorate('s3Upload', async () => { throw new Error('S3 not configured') })
    app.decorate('s3PresignGet', async () => { throw new Error('S3 not configured') })
  ...
  app.decorate('s3Upload', async (key: string, body: Buffer, contentType?: string) => {
