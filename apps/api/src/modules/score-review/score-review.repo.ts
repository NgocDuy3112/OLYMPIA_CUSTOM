...
export interface ScoreReviewRepo {
    ...
    listByMatch(matchId: string, status?: string): Promise<ScoreReviewRow[]>;
...
export const drizzleScoreReviewRepo: ScoreReviewRepo = {
    ...
    async listByMatch(matchId: string, status?: string): Promise<ScoreReviewRow[]> {
        const conditions = [eq(scoreReviews.matchId, matchId)];
        if (status) conditions.push(eq(scoreReviews.status, status));
        return db
            .select()
            .from(scoreReviews)
            .where(and(...conditions))
            .orderBy(desc(scoreReviews.createdAt))
            .limit(100);
    },
...
export function createInMemoryScoreReviewRepo(
    ...
    return {
        ...
        async listByMatch(matchId: string, status?: string) {
            return rows
                .filter(
                    (r) =>
                        r.matchId === matchId &&
