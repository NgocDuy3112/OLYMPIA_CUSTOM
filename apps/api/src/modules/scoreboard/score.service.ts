...
export async function persistScoreDeltas(
  ...
  const matchRows = await db.select({ id: matches.id })
    .from(matches)
  ...
  const questionRows = await db.select({ id: questions.id })
    .from(questions)
    .where(and(
    ...
    .limit(1)
  if (questionRows.length === 0) throw new Error(`Question not found: ${questionCode}`)

  const userCodes = [...new Set(deltas.map((delta) => delta.userCode))]
  const userRows = await db.select({ id: users.id, userCode: users.userCode })
    .from(users)
    .where(eq(users.isDeleted, false))
  const userIds = new Map(userRows.filter((user) => userCodes.includes(user.userCode)).map((user) => [user.userCode, user.id]))
