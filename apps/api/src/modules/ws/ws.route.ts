...
export async function wsRoute(app: FastifyInstance) {
  app.get('/ws/:matchCode', { websocket: true }, async (socket: WebSocket, request) => {
    ...
    const matchRows = await db.select({ tournamentFormat: matches.tournamentFormat })
      .from(matches)
    ...
    const tournamentFormat = toTournamentFormat(matchRows[0].tournamentFormat)
    if (!tournamentFormat) {
