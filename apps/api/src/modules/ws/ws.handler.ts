...
async function handleEngineAction(conn: WsConnection, data: WsMessage): Promise<void> {
  const engine = getEngine(conn.tournamentFormat)
  const valkey = manager.valkey
  if (!valkey) return

  const stored = await matchState.getField<Record<string, unknown>>(valkey, conn.matchCode, 'engine_state')
  ...
  const action = {
    ...
    phase: (data.phase as string) || (state as any).currentPhase || engine.phases[0],
  ...
  if (result.scoreDeltas.length > 0) {
    try {
      ...
      await manager.sendToUser(conn.matchCode, conn.userCode, {
        ...
        message: error instanceof Error ? error.message : 'Failed to persist score',
      })
      return
    }
  }

  await matchState.setField(valkey, conn.matchCode, 'engine_state', result.value)
