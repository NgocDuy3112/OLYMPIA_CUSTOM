...
export async function createSession(
  ...
  await valkey.set(`${SESSION_PREFIX}${sid}`, JSON.stringify(data), 'EX', SESSION_TTL)
  return sid
}

export async function getSession(valkey: any, sid: string): Promise<SessionData | null> {
...
async function exchangeCode(code: string) {
  ...
  if (!resp.ok) throw new AppError(401, 'Failed to exchange Google authorization code')
...
async function fetchGoogleUserInfo(accessToken: string) {
  ...
  return resp.json() as Promise<{ sub: string; email: string; name: string; picture?: string }>
}
  ...
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (existing.length > 0) {
      ...
      await db.update(users)
        .set({ googleId: googleUser.sub, avatarUrl: googleUser.picture ?? user.avatarUrl, updatedAt: new Date() })
    ...
    const env = getEnv()
    const frontendUrl = env.CORS_ORIGINS === '*'
      ? 'http://localhost:5173'
...
export function getMe(app: FastifyInstance) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!sid) {
      return reply.code(401).send({ status: 'error', message: 'Not authenticated', data: null })
    }
    const session = await getSession(app.valkey, sid)
    if (!session) {
      return reply.code(401).send({ status: 'error', message: 'Session expired', data: null })
    }
...
export function logout(app: FastifyInstance) {
  return async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.clearCookie(COOKIE_NAME, { path: '/' })
