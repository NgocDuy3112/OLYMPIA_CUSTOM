...
async function getField<T = unknown>(valkey: Redis, matchCode: string, field: string): Promise<T | null> {
...
async function setField(valkey: Redis, matchCode: string, field: string, value: unknown): Promise<void> {
...
async function delFields(valkey: Redis, matchCode: string, ...fields: string[]): Promise<void> {
...
async function getAllFields(valkey: Redis, matchCode: string): Promise<Record<string, unknown>> {
...
export const matchState = {
  ...
  // ── VeDich powers ──
  getUsedPowers: async (valkey: Redis, matchCode: string): Promise<Record<string, string>> => {
  ...
  setUsedPower: async (
    valkey: Redis, matchCode: string, userCode: string, power: string,
  ...
  getSnapshotMessages: async (valkey: Redis, matchCode: string): Promise<Record<string, unknown>[]> => {
    const data = await getAllFields(valkey, matchCode)
    const replayOrder = [
      'qualifier_round', 'vd_selected_chung', 'vd_selected_rieng',
      'vdc_meta', 'vdr_meta', 'vdc_question_states', 'vdr_question_states',
      'current_question', 'timer', 'video', 'answers',
      'vd_selection_update', 'keyword_info', 'keyword_clues_locked',
      'keyword_answers', 'keyword_answer', 'qualifier_round_result',
    ]
    const messages: Record<string, unknown>[] = []
    for (const field of replayOrder) {
      const value = data[field]
      if (!value) continue
      if ((field === 'vdc_question_states' || field === 'vdr_question_states') && typeof value === 'object') {
        for (const m of Object.values(value as Record<string, unknown>)) {
          if (typeof m === 'object' && m !== null) messages.push(m as Record<string, unknown>)
        }
