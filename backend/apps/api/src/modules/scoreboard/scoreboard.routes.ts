...
import { db, records, users, matchPlayerPositions, matches } from '@olympia/db'
...
export async function scoreboardRoutes(app: FastifyInstance) {
  ...
  app.get('/scoreboard/:matchCode', async (request, reply) => {
    ...
    const scoreRows = await db.select({
      userCode: users.userCode,
      totalPoints: sql<number>`COALESCE(SUM(${records.points}), 0)`.as('total_points'),
    })
      .from(records)
    ...
    const scoreMap = new Map(scoreRows.map((r) => [r.userCode, Number(r.totalPoints)]))
    ...
    return reply.send({ status: 'success', message: 'OK', data: { scoreboard } })
  })
}
