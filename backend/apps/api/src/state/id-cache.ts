...
import { db, users, matches, questions } from '@olympia/db'

const CACHE_TTL = 3600 // 1 hour

function matchKey(code: string) { return `id:match:${code}` }
function userKey(code: string) { return `id:user:${code}` }
function questionKey(code: string) { return `id:question:${code}` }

export async function resolveMatchId(valkey: Redis, matchCode: string): Promise<string | null> {
...
export async function resolveUserId(valkey: Redis, userCode: string): Promise<string | null> {
...
export async function resolveQuestionId(valkey: Redis, questionCode: string): Promise<string | null> {
  const cached = await valkey.get(questionKey(questionCode))
  if (cached) return cached

  const row = await db.select({ id: questions.id })
    .from(questions)
    .where(and(eq(questions.questionCode, questionCode), eq(questions.isDeleted, false)))
    .limit(1)
...
export async function resolveBuzzIds(
  ...
  userCode: string,
  questionCode: string,
): Promise<{ matchId: string | null; playerId: string | null; questionId: string | null }> {
