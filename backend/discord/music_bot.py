"""Discord Music Bot — plays background music and timer BGM on game events.

Merged from music_bot.py (phase BGM) and bgm_bot.py (timer BGM).
...
async def _play(guild: discord.Guild, file_path: str) -> None:
    ...
    if not vc:
        logger.error("Could not obtain voice client")
        return
...
async def _valkey_listener() -> None:
    ...
    logger.info(f"Music Bot listening on channel '{match_code}'")
