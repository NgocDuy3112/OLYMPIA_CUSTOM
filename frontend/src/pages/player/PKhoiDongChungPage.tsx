		...
		switch (msg?.type) {
			...
			case "answer": {
				...
				if (user_code && user_code !== playerCode && answer_text) {
					console.info(`[KDC ANSWER SYNC] Player received answer from user=${user_code} answer=${answer_text} ts=${timestamp}`);
					setPlayers((prev) =>
						prev.map((p) =>
					...
					console.warn(`[KDC ANSWER SYNC] Player received empty answer: user_code=${user_code} answer_text=${answer_text}`);
				}
				break;
			}
