...
const SeasonListPage: React.FC = () => {
  ...
  const handleDelete = async (seasonCode: string) => {
    if (!confirm("Bạn có chắc chắn muốn xóa mùa giải này?")) {
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/seasons/${seasonCode}`, {
        method: "DELETE",
        credentials: "include",
      });

      if (response.ok) {
        setSeasons((prev) => prev.filter((s) => s.seasonCode !== seasonCode));
      }
    } catch (err) {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">Mùa Giải</h1>
          <p className="text-gray-400 text-sm mt-1">Quản lý các mùa giải đấu</p>
        </div>
        <button
      ...
      {seasons.length === 0 ? (
        <div className="card text-center py-12">
          ...
          <p className="text-gray-400 mb-4">Chưa có mùa giải nào</p>
          <button
            onClick={() => navigate("/admin/seasons/create")}
            ...
            >
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                ...
                <div
                  ...
                  onClick={() => navigate(`/admin/seasons/${season.seasonCode}`)}
                >
                  ...
                  <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs sm:text-sm text-gray-400">
                    ...
                    <span className="flex items-center gap-1">
                      <Users size={14} />
                      {season.tournamentFormat.toUpperCase()}
                    </span>
                  </div>
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    onClick={() => navigate(`/admin/seasons/${season.seasonCode}/edit`)}
                  ...
                  <button
                    onClick={() => handleDelete(season.seasonCode)}
