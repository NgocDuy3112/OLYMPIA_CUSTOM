...
const INITIAL_FORM_DATA: SeasonFormData = {
  seasonName: "",
  description: "",
...
const SeasonFormPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState<SeasonFormData>(INITIAL_FORM_DATA);
  const [isLoading, setIsLoading] = useState(isEditing);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Load season data if editing
  useEffect(() => {
    if (!code) return;
      ...
      try {
        ...
        if (data.status === "success" && data.data) {
          ...
          setFormData({
            ...
            tournamentFormat: season.tournamentFormat || "oc3",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.seasonName.trim()) {
      setError("Tên mùa giải là bắt buộc");
      return;
    }
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white">
            {isEditing ? "Chỉnh sửa mùa giải" : "Tạo mùa giải mới"}
          </h1>
          <p className="text-gray-400 text-sm mt-1">
            {isEditing ? "Cập nhật thông tin mùa giải" : "Điền thông tin để tạo mùa giải mới"}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card !p-6 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-white mb-2">
            Tên mùa giải <span className="text-red-400">*</span>
          </label>
          <input
            type="text"
            name="seasonName"
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
          ...
          >
            ...
            <span>{isEditing ? "Cập nhật" : "Tạo mùa giải"}</span>
          </button>
        </div>
      </form>
    </div>
