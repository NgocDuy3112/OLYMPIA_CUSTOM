...
const SeasonDetailPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchSeason = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/seasons/${code}`, {
          credentials: "include",
        });
  ...
  const handleAddPlayer = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/seasons/${code}/players`, {
        method: "POST",
        headers: {
      ...
      // Refresh season data
      const refreshResponse = await fetch(`${API_BASE_URL}/seasons/${code}`, {
        credentials: "include",
      });
  ...
  const handleRemovePlayer = async (userCode: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa thí sinh ${userCode} khỏi mùa giải?`)) {
      return;
    }

    try {
      const response = await fetch(
        `${API_BASE_URL}/seasons/${code}/players/${userCode}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  if (!season) {
    return (
      <div className="flex justify-center items-center h-64">
        <p className="text-gray-400">Không tìm thấy mùa giải</p>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span
              className={`px-2 py-0.5 rounded text-xs font-medium text-white ${STATUS_COLORS[season.status] || "bg-gray-500"}`}
            >
              {STATUS_LABELS[season.status] || season.status}
            </span>
          </div>
          <p className="text-gray-400 text-sm mt-1">{season.seasonCode}</p>
        </div>
        <button
          onClick={() => navigate(`/admin/seasons/${code}/edit`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card !p-6">
            <h2 className="text-lg font-bold text-white mb-4">Thông tin mùa giải</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2 text-gray-300">
                <Trophy size={16} className="text-blue-400" />
                <span>Format: {season.tournamentFormat.toUpperCase()}</span>
              </div>
              <div className="flex items-center gap-2 text-gray-300">
                <Calendar size={16} className="text-blue-400" />
                <span>
                  {season.startDate || "Chưa đặt"} - {season.endDate || "Chưa đặt"}
                </span>
              </div>
              {season.venue && (
                <div className="flex items-center gap-2 text-gray-300">
              ...
              {season.maxPlayers && (
                <div className="flex items-center gap-2 text-gray-300">
                  ...
                  <span>Tối đa {season.maxPlayers} thí sinh</span>
                </div>
              )}
            </div>
            ...
            {season.notes && (
              <div className="mt-4 pt-4 border-t border-white/10">
                <p className="text-gray-400 text-xs">Ghi chú: {season.notes}</p>
              </div>
            )}
          </div>
