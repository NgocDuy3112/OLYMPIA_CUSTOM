...
const AdminButPhaView = () => {
  ...
  } = useGameRound({ round: "bp", questionPrefix: QUESTION_PREFIX, timeLimit: TIME_LIMIT, timerPhase: "bp" });
  ...
  }, [isTimerRunning, currentQuestionIndex, setCurrentQuestionIndex, loadQuestion, sendQuestionToPlayers, clearQuestion]);
        ...
        return (
          <button key={idx} type="button" disabled={isTimerRunning}
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-md text-sm font-bold transition-colors ${
              isActive ? "bg-blue-300 text-blue-900 border border-blue-200"
  ...
  return (
    <ANewBaseLayout
      title="BỨT PHÁ" players={players} selectedPlayerCodes={selectedPlayerCodes}
      ...
      playerActions={
        <>
          ...
          <AControlButton onClick={() => calculateAndBroadcastScore("bp_resolve")} disabled={selectedPlayerCodes.length === 0 || hasAddedScore || isTimerRunning}>
    ...
    >
      <AQuestionBoard title="BỨT PHÁ" question={currentQuestion} timerDuration={timer}
...
const PlayerButPhaView = () => {
  ...
  const {
    isConnected, sendMessage, timer, timeLimit, getElapsedSeconds,
    currentQuestion, currentQuestionIndex, players, setPlayers, showAnswers,
    videoPlayState, timerHasStarted,
  } = usePlayerRound();
  ...
  useEffect(() => {
    return () => { if (submitTimeoutRef.current) window.clearInterval(submitTimeoutRef.current); };
  }, []);

  const handleSubmitAnswer = useCallback(async () => {
    const trimmed = answer.trim();
    if (!trimmed || submitDisabledTemporarily || !isConnected || !currentQuestion.questionCode || !timerHasStarted || timer <= 0) return;

    const elapsed = getElapsedSeconds();
    const ts = Math.max(0, Math.min(timeLimit, elapsed));

    setSubmitDisabledTemporarily(true);
    setSubmitDisableSecondsLeft(3);
    ...
    submitTimeoutRef.current = window.setInterval(() => {
      setSubmitDisableSecondsLeft(prev => {
        if (prev <= 1) { if (submitTimeoutRef.current) { window.clearInterval(submitTimeoutRef.current); submitTimeoutRef.current = null; } setSubmitDisabledTemporarily(false); return 0; }
        return prev - 1;
      });
    }, 1000);

    setPlayers(prev => prev.map(p =>
      p.playerCode === playerCode ? { ...p, playerLastAnswer: trimmed, playerTimestamp: Number(ts.toFixed(3)) } : p
    ));

    try {
      await submitAnswer({ user_code: playerCode, match_code: matchCode, question_code: currentQuestion.questionCode, answer_text: trimmed, has_buzzed: false, timestamp: ts });
      await sendMessage({ type: "player_answer", user_code: playerCode, question_code: currentQuestion.questionCode, answer_text: trimmed, timestamp: ts });
    } catch (error) { console.warn("Failed to submit answer:", error); }
    setAnswer("");
  }, [answer, currentQuestion.questionCode, getElapsedSeconds, isConnected, matchCode, playerCode, sendMessage, setPlayers, submitDisabledTemporarily, timeLimit, timer, timerHasStarted]);
  ...
  const answerPlaceholder = !currentQuestion.questionCode ? "Chờ admin chọn câu hỏi..."
    : !timerHasStarted ? "Chờ admin bắt đầu tính giờ..."
    : isTimerExpired ? "Thời gian đã hết!"
    : submitDisabledTemporarily ? `Vui lòng đợi trong ${submitDisableSecondsLeft} giây`
  ...
  return (
    <PBasePageLayout players={displayPlayers} currentPlayerCode={playerCode}>
      <PQuestionBoard title="BỨT PHÁ" question={currentQuestion} timerDuration={timer}
        controls={{ variant: 'numbers', count: 5, activeIndices: currentQuestionIndex > 0 ? [currentQuestionIndex - 1] : [] }}
        videoPlayState={videoPlayState} hideMediaUntilPlayed
      />
      <PAnswerBox answer={answer} setAnswer={setAnswer} isDisabled={isSubmissionDisabled}
        onSubmit={handleSubmitAnswer} placeholderString={answerPlaceholder}
      />
    </PBasePageLayout>
  );
};
