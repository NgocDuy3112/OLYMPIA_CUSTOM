...
**Direction**: Player → Server  
**Purpose**: Submit keyword guess in Giải Mã round. Players may submit **once only**; a confirmation dialog is shown before the message is sent. Persisted to the answers REST API and broadcast via WebSocket for the admin to track.  
**Sent by**: `PGiaiMaPage`
...
|-------|------|-------------|
| `user_code` | string | Player's user code |
| `keyword_text` | string | Keyword text entered by the player |
| `timestamp` | number | Always `0` (keyword submission is not time-scored) |
...
**Direction**: Admin → Server  
**Purpose**: Send metadata for Về Đích CÁ NHÂN questions (Individual Stage)  
...
**Triggered by**: Record creation via API, or admin manual score adjustment via `PATCH /scoreboard/adjust`
...
> **Note**: When an admin adjusts a player's score via the `PATCH /scoreboard/adjust` API endpoint, the admin frontend also broadcasts a `send_players_info` event with the full updated scoreboard so all clients can resync.
...
**Purpose**: Notify all players that keyword input is now locked and cannot be changed  
**Triggered by**: `AGiaiMaPage` when all 8 clues have been revealed **or** when all players have already submitted their keyword
...
### reveal_keyword_answer

**Purpose**: Reveal the correct keyword answer to all players  
**Triggered by**: Admin pressing "MỞ TỪ KHOÁ" in `AGiaiMaPage`. The answer is fetched from question code `OC3_Q_GM_KEY`.
...
### send_keyword_answers

**Purpose**: Show all players' keyword submissions on the player screen (displayed as the last-answer field). Only sent after the keyword answer has been revealed.  
**Triggered by**: Admin pressing "HIỆN TỪ KHOÁ" in `AGiaiMaPage`

**Payload**:
```typescript
{
  type: "send_keyword_answers",
  answers: Array<{
    user_code: string,
    content: string,
    timestamp: number
...
```json
{
  "type": "send_keyword_answers",
  "answers": [
    { "user_code": "OC_U001", "content": "HÒA BÌNH", "timestamp": 0 },
    { "user_code": "OC_U002", "content": "TỰ DO", "timestamp": 0 }
  ]
}
```
...
### Khởi Động CÁ NHÂN (Individual Warm-up - KDR)
...
### Về Đích CÁ NHÂN (Final Individual Stage - VDR)
...
|-------|-----------|---------|
| `player_online` | Player → Server | Connection |
| `answer` | Player → Server | Submit clue answer |
| `keyword_submit` | Player → Server | Submit keyword guess (one-time, requires confirmation) |
| `send_players_info` | Admin → Server | Update scores |
| `send_question` | Admin → Server | Send clue question |
| `start_the_timer` | Admin → Server | Start timer |
| `send_answers_to_players` | Admin → Server | Show clue answers |
| `keyword_locked` | Admin → Server | Lock keyword input (all clues opened or all players submitted) |
| `reveal_keyword_answer` | Admin → Server | Reveal correct keyword answer |
| `send_keyword_answers` | Admin → Server | Show all players' keyword submissions |
| `navigate` | Admin → Server | Navigate |
...
// Giải Mã – keyword events
interface KeywordSubmitEvent extends WSEvent {
  type: "keyword_submit";
  user_code: string;
  keyword_text: string;
  timestamp: number;
...
interface KeywordLockedEvent extends WSEvent {
  type: "keyword_locked";
}

interface RevealKeywordAnswerEvent extends WSEvent {
  type: "reveal_keyword_answer";
  answer: string;
}

interface SendKeywordAnswersEvent extends WSEvent {
  type: "send_keyword_answers";
  answers: Array<{
    user_code: string;
    content: string;
